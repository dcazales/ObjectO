           	...
           	do {
                  ...
                  //Choices begin *_*_*_*_*_*_*_*_*_*_*_*_*_*_*_*_*_*_*_*_*_*_*
                   ...
                   MemberWriter.writeMembersToScreen(members);
                       ...
                       a = sc.next().trim();
                       //Will add a new member to the file.
                	   ...
                	   System.out.print("(T)ext, (B)inary, or (X)ML?");
                     	String textOption = option.nextLine();
                     	System.out.println("Enter then name of the input file: ");
                     	filename = option.nextLine();
                     	
                     	if (textOption.equals("B")) {
                     		MemberReader.readMembersFromTextFile(filename);
                     	    System.out.println("Members were loaded successfully.");

                     	} else if (textOption.equals("T")) {
                     		 MemberReader.readMembersFromBinary(filename);
                     	    System.out.println("Members were loaded successfully.");
                     	
                     	}else if (textOption.equals("X")) {
                     		MemberReader.readMembersFromXML(filename);
                     	    System.out.println("Members were loaded successfully.");
                     	}      
                   }
                   
                   else if (choice == 5) {
                	   System.out.print("Here are the Memebers with their scores: \n");
                		Assessor assessor = new Assessor();
                		ArrayList<InsuranceScore> insuranceScore = assessor.insuranceScore(members);
                		InsuranceScoreWriter.writeMembers(insuranceScore);
                       ...
                       filename = option.nextLine();
                       if(InsuranceScoreWriter.writeMembersToJSON(filename,members)) {
                    	   System.out.println("Printed sucessfully");
                       } else {
                    	   System.out.println("Something went wrong, sorry feature not available");
                       }
